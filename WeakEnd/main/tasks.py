...
app = Celery('tasks',  backend='rpc://', broker='amqp://harry595:hh237237@ec2-54-180-91-153.ap-northeast-2.compute.amazonaws.com//')
...
def checkvuln(urls,cookies,level,new_id):
    ...
    for url in urls:
        ...
        for get_list in get_lists:
            ...
            get_count+=1
            #current_task.update_state(state='PROGRESS',meta={'process_percent': taskflag*taskcal+little_task*get_count})

        taskflag+=1
        current_task.update_state(state='PROGRESS',meta={'process_percent': taskflag*taskcal})
        #little_task = taskcal/len(zetanize_lists)
        post_count=0
        for post_list in zetanize_lists:
            ...
            if(sqli_result!=None and sqli_result!=False):
                vuln_list.append(sqli_result)
            post_count+=1
            #current_task.update_state(state='PROGRESS',meta={'process_percent': taskflag*taskcal+little_task*post_count})
        taskflag+=1
        current_task.update_state(state='PROGRESS',meta={'process_percent': taskflag*taskcal})
